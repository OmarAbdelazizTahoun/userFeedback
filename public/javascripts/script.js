...
function createIssuesDistributionChart(){
    ...
    let issuesDistChart = new Chart(issuesDistributionCtx, config);
}
...
function createChart() {
    ...
    let dates = labels.map((label) => {
        let release = releases.find((item) => item.tagName == label);
        // format the date as a string DD/MM/YYYY
        return release ? release.publishedAt.toLocaleDateString() : '';
    });
...
function createArtifactsCharts(){
    ...
    let dates = labels.map((label) => {
        let release = releases.find((item) => item.tagName == label);
        // format the date as a string MMM YYYY
        return release ? release.publishedAt.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : '';
    });
...
function createPackageSmellsChart(){
    ...
    // // sum the smells for each level
    let packageSmells = ["Unstable_Dependency", "Package_Cyclic_Dependency"]
    let packageSmellsValues = filteredSmells.map((item) => {
        return packageSmells.reduce((acc, smell) => acc + item[smell], 0);
    });

    datasets.push({
        label: 'Package Level Smells',
        data: packageSmellsValues,
        borderColor: 'rgba(54, 162, 235, 1)',
        backgroundColor: 'rgba(54, 162, 235, 0.2)',
    });
...
function createMethodSmellsChart(){
    ...
    let methodSmells = ["Brain_Method", "Feature_Envy", "Long_Parameter_List", "Shotgun_Surgery", "Long_Method"];
    let methodSmellsValues = filteredSmells.map((item) => {
        return methodSmells.reduce((acc, smell) => acc + item[smell], 0);
    });

    datasets.push({
        label: 'Method Level Smells',
        data: methodSmellsValues,
        borderColor: 'rgba(54, 162, 235, 1)',
        backgroundColor: 'rgba(54, 162, 235, 0.2)',
    });
...
function createClassSmellsChart(){
    ...
    let classSmells = Object.keys(filteredSmells[0]).filter((item) => !["version", "Total", "Distinct_Count", "Brain_Method", "Feature_Envy", "Long_Parameter_List", "Shotgun_Surgery", "Long_Method", "Unstable_Dependency", "Package_Cyclic_Dependency"].includes(item));
    let classSmellsValues = filteredSmells.map((item) => {
        return classSmells.reduce((acc, smell) => acc + item[smell], 0);
    });

    datasets.push({
        label: 'Class Level Smells',
        data: classSmellsValues,
        borderColor: 'rgba(54, 162, 235, 1)',
        backgroundColor: 'rgba(54, 162, 235, 0.2)',
    });
